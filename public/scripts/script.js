...
function askQuestion(ignoreSignal) {
  ...
  startTyping(Questions[CurrentQuestionNumber].question);
  setTimeout(() => {
...
socket.on("info", (i) => {
  ...
  if (i.paired) {
    ...
    elseif (appStatusElement.textContent === "Desconectado")
      Toast.fire({
        icon: "warning",
...
function openSettingsModal() {
  Swal.fire({
    ...
    preConfirm: () => {
      ...
      return { backgroundMusic, soundEffect, inverter };
    },
  }).then((result) => {
    ...
    preConfirm: () => {
      ...
      return new Promise((resolve) => {
        ...
        reader.onload = function (e) {
          ...
          socket.emit("uploadFile", data, (status) => {
            if (status.success)
              Swal.fire("Sucesso!", "Arquvo enviado!", "success");
...
function InitializeGame() {
  ...
  socket.emit("list-questionaries", (questionaries) => {
    let qn = {};
